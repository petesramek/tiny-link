...
public:
    ...
    void update() {
        ...
        while (_hw->available() > 0) {
            ...
            switch (_state) {
                case TinyState::WAIT_SOH: if (c == SOH) { _status = TinyStatus::STATUS_OK; _state = TinyState::IN_TYPE; } break;
                ...
                case TinyState::WAIT_H_CHK: if (c == checksum(_hBuf, 3)) _state = TinyState::WAIT_STX; else reset(TinyStatus::ERR_CRC, NAK); break;
                case TinyState::WAIT_STX: if (c == STX) { _pIdx = 0; _state = TinyState::IN_PAYLOAD; } else reset(TinyStatus::ERR_CRC, CAN); break;
                case TinyState::IN_PAYLOAD: if (_pIdx < _expectedLen) _pBuf[_pIdx++] = c; if (_pIdx >= _expectedLen) _state = TinyState::WAIT_ETX; break;
                case TinyState::WAIT_ETX: if (c == ETX) _state = TinyState::WAIT_P_CHK; else reset(TinyStatus::ERR_CRC, CAN); break;
                case TinyState::WAIT_P_CHK: if (c == checksum(_pBuf, _expectedLen)) _state = TinyState::WAIT_ACK; else reset(TinyStatus::ERR_CRC, NAK); break;
                case TinyState::WAIT_ACK: if (c == ACK) { memcpy(&_data, _pBuf, sizeof(T)); _hasNew = true; _stats.packets++; } _state = TinyState::WAIT_SOH; break;
            }
        }
    }
